...
| M4 | Dashboard | Chart has no point cap (table capped at 500, chart not); multi-hour session at "all" zoom re-renders 10k+ points every 2s → jank/freeze. *(reviewer)* | `.../curelog-phase1-dashboard.html:~1532` | **Fixed (dash 1.7.22 / mobile 1.7.20-m)** — stride-decimate to 800 (dash) / 400 (mobile) points, keep latest. Also fixed dash live "No data"/duration-stuck on shots loaded before data exists. |
