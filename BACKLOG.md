...
| E2 | Android | **Layer 3 (deferred, needs design):** (a) unmistakable "NOT RECORDING — press START" state while connected but not started; (b) **append/resume workflow** — when a shot has to be restarted mid-cure (tablet battery dies, app reload), allow *intentionally* continuing an existing shot instead of overwriting/duplicating it. Must NOT block legitimate resume. Design the start-fresh-vs-resume UX with Dan before building. | Open — design |
