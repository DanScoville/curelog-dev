...
1. **Pressure parsing (S1 / TDWLB5):** `sint16 big-endian × pressScale`, where `pressScale` comes from the `0x2904` descriptor (second byte read as `sint8` exponent → `10^exp`). This is the GATT Pressure Measurement value (attr `0x001c`) and is **big-endian in BOTH `readValue()` and NOTIFY** — they deliver the same GATT attribute. The datasheet "watchlist" parsing is **little-endian but applies to advertisement packets only** (attr `0x2aff`) and must NOT be used here. Confirmed against the *TDWLB-G2G5 Attribute Database V5* (Transducers Direct, CONFIDENTIAL — kept off-repo). Do NOT add a little-endian fallback to the GATT parse — the notify handler used to auto-guess BE/LE and could silently report a wrong PSI (fixed in 2.1.63-m). (Note: the S2 WETS02 temperature parse *is* `sint16 LE × 0.1` — that's a different sensor, leave it.)
2. **Sensor hardware is fixed:** S1 pressure = **TDWLB5 Gen5** (Transducers Direct, 0–50 PSI, BLE — NOTIFY preferred, polling fallback). S2 temperature = **WETS02** (Phoenix Sensors, BLE NOTIFY). Do not swap models or parsing without instruction.
3. **Pressure runs in NOTIFY (preferred), polling as fallback.** Letting the TDWLB5 broadcast pressure via NOTIFY reaches ~1 Hz; request/response polling capped ~0.5 Hz. The connect flow enables NOTIFY when the characteristic advertises it and falls back to `readValue()` (~250ms) otherwise. **Do-not-break caveat:** Chrome mis-routes notifications when two *connected devices share the same characteristic UUID*. This is safe **today** because there is exactly one pressure sensor (S1 `835ab4c0…`) and one temperature sensor (S2 WETS02 `A764…`), distinct UUIDs. **If a future build connects two TDWLB5 pressure sensors** (both `835ab4c0…`, e.g. inlet+outlet), that config must NOT use NOTIFY on pressure — force polling, or guarantee airtight per-device `e.target` routing. Keep IIFE closures / `e.target` matching wherever same-UUID characteristics are involved.
